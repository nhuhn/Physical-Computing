...
void loop(){
  int lightLevel = analogRead(lightPin)*5; //Read the
                                          // lightlevel
 lightLevel = map(lightLevel, 0, 900, 0, 255);//change map from 900 to 255

  byte tempColor[] = {lightLevel*2,lightLevel*2.5,lightLevel}; //the RGB (red, gren blue) value for a color to display
